...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%if category="F" then response.write "自學療程" else response.write  "自學軟體/補充教材"%>&nbsp;簽到作業</TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD>日期</TD><TD>開始時間</TD><TD>結束時間</TD><TD>分鐘</TD><TD>項目</TD><TD>預約人</TD><TD>預約狀態</TD>
			</TR>
			<TR class="inputlabel">
			<TD></TD>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<TD>&nbsp;&nbsp;<%=replace(replace(replace(rs("yn"),"Y","<font color=""blue"">已預約</font>"),"N","<font color=""red"">取消</font>"),"A","<font color=""#339900"">預約未到</font>")%></TD>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<%if rs("yn")="A" then%>
					<input  type="button" value="取消預約未到" <%if rs("yn")="N" then response.write "disabled" end if%>  onclick="err_absence_item('<%=rs("id")%>','<%=rs("tid")%>','<%=rs("item")%>','<%=rs("summin")%>')"   class="inputbutton"  style='color:#3300CC;font-size=9pt' onMouseOver="this.style.color='#3399FF'" onMouseOut="this.style.color='#3300CC'">
				<%else%>
					<%if  rs("signin")<>"" then %>
					<input  type="button" value="取消報到" <%if rs("yn")="N" then response.write "disabled" end if%>  onclick="err_record_item('<%=rs("id")%>','<%=rs("tid")%>','<%=rs("item")%>','<%=rs("summin")%>')"   class="inputbutton"  style='color:FF6600;font-size=9pt' onMouseOver="this.style.color='#FF0000'" onMouseOut="this.style.color='#FF6600'">
					<%else%>
					<input  type="button" value="預約未到" <%if rs("yn")="N" then response.write "disabled" end if%>  onclick="absence_record_item('<%=rs("id")%>','<%=rs("tid")%>','<%=rs("item")%>','<%=rs("summin")%>')"   class="inputbutton"  style='color:#3300CC;font-size=9pt' onMouseOver="this.style.color='#3399FF'" onMouseOut="this.style.color='#3300CC'">
					<input  type="button" value="報到登入" <%if rs("yn")="N" then response.write "disabled" end if%> onclick="signin_record_item('<%=rs("id")%>','<%=rs("tid")%>','<%=rs("item")%>','<%=rs("summin")%>')" class="inputbutton"  style='color:009900;font-size=9pt' onMouseOver="this.style.color='#00cc00'" onMouseOut="this.style.color='#009900'">
					<%end if%>
				<%end if%>
				</TD>
			</TR>

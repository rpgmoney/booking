...
if scope<>"" then
	...
	if instr(scope,"1") <>0 then
		sql = sql &  " select slevel,grade   from boo_book_T_M  where yn='Y' and signin is not null "
	...
	if instr(scope,"2") <>0 then
		...
		sql = sql &  " select slevel,grade  from boo_book_software  where yn='Y' and signin is not null "
		if sdate<>"" and edate="" then
			sql = sql & " and  Cast(bdate as int) >= '" & sdate& "'  "
		end if
		if sdate="" and edate<>"" then
	...
	if instr(scope,"3") <>0 then
		if sql<>"" then sql=sql & " union all " end if
		sql = sql &  " select slevel,grade  from boo_book_lecture a inner join boo_lecture b on a.lid=b.id where yn='Y' and signin is not null "
		if sdate<>"" and edate="" then
	...
	if sql<>"" then
		sqlc =  "select a.name,grade,isnull(b.cnt,0) as cnt from boo_slevel a left join "
		sqlc = sqlc & " ("
		sqlc = sqlc  & "select slevel,grade ,count(*) as cnt  from ( " & sql & " ) a group by slevel,grade "
		sqlc = sqlc & " ) b on a.name=b.slevel where a.flag='S' order by a.seq,b.grade"
...
<%
	if not rs.eof then 
%>
<P align="center" class="inputlabel"><font size="4">年級統計分析表</font></P>

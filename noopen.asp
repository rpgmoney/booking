...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR>
		<TD></TD><TD valign="top">
				...
				<input  type="button"  onclick="window.location='noopenadd.asp'" value="新增" class="inputbutton">
		</TD>
	</TR>
	<%
		set rs = server.CreateObject("adodb.recordset")
		sql = "select * from boo_noopen a where 1=1  "
		
		
		
		sql = sql & " order by noopendate  "
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD>不開放日期</TD><TD>新增日期</TD><TD>新增人員</TD> <TD>開放否</TD><TD></TD><TD></TD>
			</TR>
			<TR>
				<TD height="1" bgcolor="#000000" colspan="11"></TD>
			</TR>
			...
			%>
			<TR bgcolor="<%=vcolor%>">
				<TD><a href="noopenedit.asp?noopendate=<%=rs("noopendate")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a></TD>
				<TD><%=rs("noopendate")%></TD>
				<TD><%=rs("initdate")%></TD>
				<TD><%=rs("inituid")%></TD> 

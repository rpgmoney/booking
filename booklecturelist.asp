...
sql="select id,date1,subject from boo_lecture where category='"&category&"' order by Cast(date1 as int ) desc "
'response.write sql
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%if category="L" then response.write "查詢已報名外語學習講座學員"  else response.write "查詢已報名處方課程學員"  end if%> </TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="booklecturelist.asp" >
	...
	<%
		...
		sql = sql & " order by  initdate desc "
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD><%if category="L" then response.write "講座"  else response.write "課程"  end if%>日期</TD><TD><%if category="L" then response.write "外語學習講座"  else response.write "處方課程"  end if%></TD><TD>報名時間</TD><TD></TD><TD></TD><TD>預約人</TD><TD>預約狀態</TD><TD></TD>
			</TR>
			<TR>
				<TD height="1" bgcolor="#000000" colspan="11"></TD>
			</TR>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<TD><%=rs("date1")& "(&nbsp;"&dic.Item(cstr(cint(weekday(NumberToDateFormat(rs("date1"))))))&"&nbsp;)"%><%=rs("stime")%>&nbsp;~&nbsp;<%=rs("etime")%></TD>
				<TD><%=left(rs("subject"),10)%><%if len(trim(rs("subject")))>10 then response.write "..." end if%></TD><TD><%=rs("initdate")%></TD>
				<TD></TD>

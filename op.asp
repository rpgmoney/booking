...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;學員口說練習紀錄維護</TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="op.asp" >
	...
	<%
		...
		sql = sql & " left join boo_op b on a.id=b.tid  where a.item='口語'  and  a.signin is not null  "
		if teachername <>"" then
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD ></TD><TD >發音</TD><TD >流利度</TD><TD >單字</TD><TD >文法</TD><TD >綜合</TD><TD ></TD>
			</TR>
			<TR>
				<TD height="1" bgcolor="#000000" colspan="11"></TD>
			</TR>
			...
			<TR bgcolor="<%=vcolor%>">
				<TD>
				<a href="opedit.asp?id=<%=rs("bookid")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a>
				</TD>
			...
			<TR bgcolor="<%=vcolor%>">
				<TD></TD><TD ></TD><TD ><%=rs("pronunciation")%></TD><TD ><%=rs("fluency")%></TD><TD ><%=rs("vocabulary")%></TD><TD ><%=rs("grammar")%></TD><TD ><%=rs("overall")%>&nbsp;</TD><TD align="right"><a href="recordprofile.asp?sid=<%=rs("sid")%>&forderid=3&sender=<%=sender%>"><img border="0"  src="images/record.gif"></a></TD>
			</TR>
			<TR>

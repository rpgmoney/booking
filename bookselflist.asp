...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;自學療程預約狀況 </TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="bookselflist.asp" >
	...
	<%
		...
		sql = "select a.*,b.item as itemname from boo_book_software a  "
		sql = sql  & "left join ( "
		sql = sql &						"select item,id from boo_self_item where yn='Y' "
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<a href="bookselfedit.asp?id=<%=rs("id")%>&category=<%=category%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a>
				<%end if%>
				</TD>

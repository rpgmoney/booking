...
set rs = server.CreateObject("adodb.recordset")
	...
	if sid<>"" and initdate<>""  then
		sql = "select a.*,b.name from boo_questionnaire_strategy  a left join boo_profile b on a.sid=b.sid where a.sid = '"&sid&"' and a.initdate='"&initdate&"' and a.yn='Y' order by a.initdate desc"
...
<tr><td>
	...
	<td>
	<% 
	for i = 5.0 to 4.4 step -0.1
		if Cstr(round(cdbl(partA)/cdbl(9),1)) = Cstr(i)  then
	%>
	...
	<%
		...
		if Cstr(round(cdbl(partB)/cdbl(14),1)) = Cstr(i) then
	%>
	...
	<%
		...
		if Cstr(round(cdbl(partC)/cdbl(6),1)) = Cstr(i)  then
	...
	<%
		...
		if Cstr(round(cdbl(partD)/cdbl(9),1)) = Cstr(i)  then
	%>
		<table width="98%" height="100%" border=1 cellpadding=2 cellspacing=0 bordercolorlight=#333333 bordercolordark=#dddddd>
	...
	<%
		...
		if Cstr(round(cdbl(partE)/cdbl(6),1)) = Cstr(i)  then
	%>
		<table width="98%" height="100%" border=1 cellpadding=2 cellspacing=0 bordercolorlight=#333333 bordercolordark=#dddddd>
	...
	<%
		...
		if Cstr(round(cdbl(partF)/cdbl(6),1)) = Cstr(i)  then
	%>
		<table width="98%" height="100%" border=1 cellpadding=2 cellspacing=0 bordercolorlight=#333333 bordercolordark=#dddddd>
	...
	%>
		<table width="98%" height="100%" border=1 cellpadding=2 cellspacing=0 bordercolorlight=#333333 bordercolordark=#dddddd>
		<tr>
		<td width="5%"><%=round(cdbl(partC)/cdbl(6),1)%></td><td width="5%">C部份</td><td width="20%">Compensati on strategies 補償策略</td><td width="70%"><%=replace(StrC,vbcrlf,"<br>")%></td>
		</tr>
		</table>
	<%
		end if

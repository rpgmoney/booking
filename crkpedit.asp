...
chant=trim(request("chant"))
rhyme=trim(request("rhyme"))
song=trim(request("song"))
...
if validate="add" then
		sql = "select * from boo_crkp where tid='"&id&"' "
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">學員詩歌饒舌紀錄維護編輯</TD>
	</TR>
	<TR >
	...
	<TR>
		...
		<TD width="100%" valign="Top">
			<form id="form1" name="form1" method="post" action="crkpedit.asp"   onsubmit="return check_input();">
			...
			<TR><TD>
				<TABLE cellSpacing=1 cellPadding=2  border=0 >
					<TR class="inputlabel">
						<TD>詩歌：</TD><TD><input type="checkbox"  value="Y"  name="chant" <%if  chant="Y" then response.write "checked" end if %> class="inputtext" ></TD>
						<TD>饒舌：</TD><TD><input type="checkbox"  value="Y"  name="rhyme"  <%if  rhyme="Y" then response.write "checked" end if %> class="inputtext" ></TD>
						<TD>歌曲：</TD><TD><input type="checkbox"  value="Y"  name="song"  <%if  song="Y" then response.write "checked" end if %> class="inputtext" ></TD>
					</TR>
				</TABLE>
			</TD></TR>
			<TR><TD>

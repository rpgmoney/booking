...
elseif validate="delete" then
	...
	if Err.number=0 then
		yn="N"
		canceldate=date()
		canceluid=session("sid")
    else
        showmessage= Err.Description
    end if
else
		sql = "select a.*,b.item as itemname  from boo_book_software a  left join boo_self_item b on a.item=b.id where a.id='"&id&"' "
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="form1" name="form1" method="post" action="bookselfedit.asp" onsubmit="return check_input();">

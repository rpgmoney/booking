...
if validate="delete_item" then
	...
	sqlm="update boo_book_software set yn='N',canceldate= Convert(varchar(10),Getdate(),111) ,canceluid='"&session("sid")&"' where id='"&delete_item&"'"
	msconn.Execute sqlm
	if category<>"F" then
		'更新處方籤的軟體已預約時間
...
function delete_record_item(vid,vtid,vitem,vsummin)
{
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%if category="S" then response.write "取消個人預約自學軟體療程" else if category="T" then response.write "取消個人預約補充教材" else if category="F" then  response.write "取消個人預約自學療程"   end if %> </TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="cancelsoftware.asp" >
	...
	<%
		...
		sql = sql & " and  Cast(a.bdate as int )  > '" & today & "' "
		if category <>"" then
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<TD>
				<a href="#" onclick="window.showModalDialog('booksoftwareview.asp?id=<%=rs("id")%>','','dialogWidth=650px;dialogHeight=500px;status=no');" ><img border="0" src="/include/lib/images/ico_info.gif"></a>
				&nbsp;&nbsp;
				<input  type="button" value="取消" <%if rs("yn")="N" then response.write "disabled" end if%> onclick="delete_record_item('<%=rs("id")%>','<%=rs("tid")%>','<%=rs("item")%>','<%=rs("summin")%>')" class="inputbutton" style='color:bc546f;font-size:9pt' onMouseOver="this.style.color='#ff6666'" onMouseOut="this.style.color='#bc546f'">
				</TD>
			</TR>
			<TR>

...
sender=server.urlencode(replace(request.servervariables("PATH_INFO")&"?page="&page& "&sid=" & sid& "&teachername=" & teachername& "&sdate=" & sdate& "&edate=" & edate&"&yn=" & yn&"&category=" & category,"%","*"))
...
'response.write dateformat("1000103")
'response.write weekday(dateformat("1000103"))
'response.end
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%if category="T" then response.write "查詢預約教師輔導療程狀況" else response.write "查詢預約小老師狀況" end if %> </TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="booked.asp" >
	...
	<%
		...
		sql = "select * from boo_book_T_M a where pid is null  "
		if yn<>"all" then
			sql = sql & " and  a.yn='"&yn&"'   "
		end if
		if category <>"" then
		...
		sql = sql & " order by bdate,btime,ptime "
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD>日期</TD><TD>時段</TD><TD>節數</TD><TD>項目</TD><TD>預約人</TD>
				<TD style="DISPLAY:<%if session("classify")="A"  then response.write "block" else response.write "none" end if %>">大專英檢</TD><TD>教師</TD><TD>預約狀態</TD><TD></TD>
			</TR>
			<TR class="inputlabel">
				<TD></TD>
				<TD colspan="8">題目</TD>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<a href="bookingtedit.asp?id=<%=rs("id")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif" title="修改資料"></a>
				<%end if%>
				</TD>
				<TD><%=rs("bdate")& "(&nbsp;"&dic.Item(cstr(cint(weekday(dateformat(rs("bdate"))))))&"&nbsp;)"%></TD>

				<TD><%=rs("btime")%></TD>
				<TD><%=replace(replace(replace(rs("timeflag"),"U","上一節"),"B","下一節"),"A","上下二節")%></TD>
				<TD><%=rs("item")%></TD><TD><%=rs("sid")%> - <%=rs("name")%>(<%=rs("department")%>，<%=rs("grade")%>)</TD>
				<TD style="DISPLAY:<%if session("classify")="A"  then response.write "block" else response.write "none" end if %>"><%=rs("score")%></TD>
				<TD><%=rs("teachername")%></TD><TD>&nbsp;&nbsp;<%=replace(replace(rs("yn"),"Y","<font color=""blue"">已預約</font>"),"N","<font color=""red"">取消</font>")%></TD>
				<TD>
				<a href="#" onclick="window.showModalDialog('bookingtveiw.asp?id=<%=rs("id")%>','','dialogWidth=650px;dialogHeight=650px;status=no');" ><img border="0" src="/include/lib/images/ico_info.gif" title="顯示預約明細"></a>
				<%if session("classify")="A" or ( session("classify")="T" and InStr(rs("teachername") ,session("sname")) )then%>
				&nbsp;<a href="recordprofile.asp?sid=<%=rs("sid")%>&sender=<%=sender%>"><img border="0"  src="images/record.gif" title="顯示歷程記錄"></a>
				<%end if%>
				</TD>
			</TR>
			<TR bgcolor="<%=vcolor%>">

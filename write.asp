...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;學員寫作紀錄維護</TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="write.asp" >
	...
	<%
		...
		sql = sql & " left join boo_write b on a.id=b.tid  where a.item='寫作'  and  a.signin is not null  "
		if teachername <>"" then
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD colspan="2">寫作主題</TD><TD colspan="5">寫作問題</TD>
			</TR>
			<TR class="inputlabel">
			...
			<TR bgcolor="<%=vcolor%>">
				<TD>
				<a href="writeedit.asp?id=<%=rs("bookid")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a>
				</TD>
			...
			<TR bgcolor="<%=vcolor%>">
				<TD></TD><TD colspan="6"><%=rs("feedback")%>&nbsp;</TD><TD align="right"><a href="recordprofile.asp?sid=<%=rs("sid")%>&forderid=4&sender=<%=sender%>"><img border="0"  src="images/record.gif"></a></TD>
			</TR>
			<TR>

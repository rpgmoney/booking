...
sql = sql & " inner join boo_read b on a.id=b.tid  where a.item='閱讀'  and  a.signin is not null  "
sql = sql & " and a.sid='"&sid&"'  order by bdate desc "
rs.Open sql,msconn,adOpenStatic,adLockReadonly

%>
<TABLE cellSpacing=1 cellPadding=2 border=0 width="100%"   name="folder_7" id="folder_7"  style="display:<%if forderid="7" then%>block<%else%>none<%end if%>">
<TR>
	<TD>
	<% if rs.EOF then
			response.write "<font   class=""norecord"" >沒有符合條件的閱讀技巧紀錄顯示</font>"
	else
		while not rs.EOF 
	...
	%>
			...
			<TABLE cellSpacing=1 cellPadding=2  border=0 width="90%"   bgcolor="#F3EFE6" style="border: 1px solid #FF66CC; padding: 0">
				<TR><TD>
					...
					<TR class="inputlabel">
						<TD colspan="2">閱讀主題：</TD>
					</TR>
					<TR >
						<TD width="10"></TD><TD><%=rs("subject")%>&nbsp;</TD>
					</TR>
					<TR class="inputlabel">
						<TD colspan="2">閱讀問題：</TD>
					</TR>
					<TR >

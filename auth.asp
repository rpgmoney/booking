...
sender=server.urlencode(replace(request.servervariables("PATH_INFO")&"?page="&page& "&sid=" & sid& "&deptname=" & deptname& "&enable=" & enable,"%","*"))
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;老師/小老師名單維護 </TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="auth.asp" >
			...
			<TR><TD></TD>
				...
				<td>
					<input type="text" name="sid" value="<%=sid%>"  maxlength="25" class="inputtext" >
				</td>
			
				<TD>
				<select name="enable" class="inputtext">
				<option value="Y" <%if enable="Y" or isnull(enable) or isempty(enable) then response.write "selected"%>>開啟</option>
				<option value="N" <%if enable="N"  then response.write "selected"%>>關閉</option>
				</select>
				</TD>
				<TD><input  type="submit" value="查詢" class="inputbutton"><input  type="button"  onclick="window.location='authadd.asp'" value="新增" class="inputbutton"></TD>
			</TR>
			</form>
			</TABLE>
		</TD>
	...
	<%
		...
		sql = "select * from boo_profile a where classify in ('T')  "
		if sid<>"" then
			sql = sql & " and  ( a.sid like '%"&sid&"%'  or a.name like '%"&sid&"%' ) "
		end if
		if enable<>"" then
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD>系所</TD><TD>職員編號或學號</TD><TD>姓名</TD><TD>權限</TD><TD>開啟否</TD><TD></TD>
			</TR>
			<TR>
				<TD height="1" bgcolor="#000000" colspan="11"></TD>
			</TR>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<TD><a href="authedit.asp?id=<%=rs("id")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a></TD>
				<TD><%=rs("department")%></TD><TD><%=rs("sid")%></TD>
				<TD><%=rs("name")%></TD><TD><%=replace(replace(rs("classify"),"T","老師"),"E","小老師")%></TD><TD><%=rs("enable")%></TD><TD></TD>
			</TR>
			<TR>

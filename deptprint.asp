...
if scope<>"" then
	...
	if instr(scope,"1") <>0 then
		sql = sql &  " select department   from boo_book_T_M  where yn='Y' and signin is not null "
	...
	if instr(scope,"2") <>0 then
		...
		sql = sql &  " select department  from boo_book_software  where yn='Y' and signin is not null "
		if sdate<>"" and edate="" then
			sql = sql & " and  Cast(bdate as int)  >= '" & sdate& "'  "
		end if
		if sdate="" and edate<>"" then
	...
	if instr(scope,"3") <>0 then
		if sql<>"" then sql=sql & " union all " end if
		sql = sql &  " select department  from boo_book_lecture a inner join boo_lecture b on a.lid=b.id where yn='Y' and signin is not null "
		if sdate<>"" and edate="" then
	...
	if sql<>"" then
		sqlc =  "select a.deptname,isnull(b.cnt,0) as cnt from ( select * from  rrm_dept where flag2='C') a left join "
		sqlc = sqlc & " ("
		sqlc = sqlc  & "select department ,count(*) as cnt  from ( " & sql & " ) a group by department "
		sqlc = sqlc & " ) b on   a.deptname=b.department order by a.seq"
...
<%
	if not rs.eof then 
%>
<P align="center" class="inputlabel"><font size="4">系科統計分析表</font></P>
...
<%
		...
		if instr(scope,"3") <>0 then
			if sqls<>"" then sqls=sqls & " union all " end if
			sqls = sqls &  " select a.id  from boo_book_lecture a inner join boo_lecture b on a.lid=b.id where yn='Y' and signin is not null "
			if sdate<>"" and edate="" then
...
'response.end
		...
		rs2.Close
		chart=round(200/totalsum,3)'圖形*數
		while not rs.EOF 
			rc=rc +1

...
if validate="signin_item" then
	...
	sqld="update boo_book_T_M set  signin= Getdate()   where id='"&signin_item&"'"
	msconn.Execute sqld
	if Err.number=0 then
		if  item<>"" and tid<>"" then
			a=UpdateItemTime_C(item,tid,"1")
		end if
		signin_item=""
    else
...
elseif  validate="err_item" then
	signin_item = trim(request("signin_item"))
	sqld="update boo_book_T_M set  signin= NULL   where id='"&signin_item&"'"
	msconn.Execute sqld
	if Err.number=0 then
		if  item<>"" and tid<>"" then
			a=UpdateItemTime_C(item,tid,"2")
		end if
		signin_item=""
    else
        showmessage= Err.Description
...
elseif validate="absence" then
	signin_item = trim(request("signin_item"))
	sqld="update boo_book_T_M set  yn='A'   where id='"&signin_item&"'"
	msconn.Execute sqld
	if Err.number=0 then
		'更新預約次數
		if  item<>"" and tid<>"" then
...
elseif validate="err_absence" then
	signin_item = trim(request("signin_item"))
	sqld="update boo_book_T_M set  yn='Y'   where id='"&signin_item&"'"
	msconn.Execute sqld
	if Err.number=0 then
		'更新預約次數
		if  item<>"" and tid<>"" then
...
function signin_record_item(vid,vitem,vtid)
{
...
function err_record_item(vid,vitem,vtid)
{
...
function absence_record_item(vid,vitem,vtid)
{
...
function err_absence_item(vid,vitem,vtid)
{
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;教師/小老師療程&nbsp;學員簽到作業</TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="signin.asp" >
			...
			<TR><TD></TD>
				...
				<option value="A" <%if yn="A" then response.write "selected" end if%>>預約未到</option>
	...
	<%
		...
		sql = "select * from boo_book_T_M a where 1=1  "
		if category <>"" then
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD>日期</TD><TD>時段</TD><TD>節數</TD><TD>項目</TD><TD>預約人</TD><TD>大專英檢</TD><TD>教師</TD><TD align="right">狀態</TD>
			</TR>
			<TR class="inputlabel">
				<TD></TD>
				<TD colspan="8">題目</TD>
			...
			<TR bgcolor="<%=vcolor%>">
				<TD>
				<a href="bookingtedit.asp?id=<%=ifnull(rs("pid"),rs("id"))%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a>
				</TD>
				...
				<TD><%=rs("item")%></TD><TD><%=rs("sid")%> - <%=rs("name")%>(<%=rs("department")%>，<%=rs("grade")%>)</TD>
				<TD><%=rs("score")%></TD>
				<TD><%=rs("teachername")%></TD>
				<TD  align="right">&nbsp;&nbsp;<%=replace(replace(replace(rs("yn"),"Y","<font color=""blue"">已預約</font>"),"N","<font color=""red"">取消</font>"),"A","<font color=""#339900"">預約未到</font>")%></TD>
			</TR>
			<TR bgcolor="<%=vcolor%>">
				<TD></TD>
				<TD colspan="8">
				<%
				if rs("item")="口語" then
					response.write rs("topic") & "/" & rs("orallevel") & "(" &  rs("oralset") & ")"
				elseif rs("item") = "簡報" then
					response.write rs("briefing")
				elseif rs("item") = "諮商" then
					response.write rs("consult")
				end if
				%>
				</TD>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<%if rs("yn")="A" then%>
					<input  type="button" value="取消預約未到" <%if rs("yn")="N" then response.write "disabled" end if%>  onclick="err_absence_item('<%=rs("id")%>','<%=rs("item")%>','<%=rs("tid")%>')"   class="inputbutton"  style='color:#3300CC;font-size=9pt' onMouseOver="this.style.color='#3399FF'" onMouseOut="this.style.color='#3300CC'">
				<%else%>
					<%if  rs("signin")<>"" then %>
					<input  type="button" value="取消報到" <%if rs("yn")="N" then response.write "disabled" end if%>  onclick="err_record_item('<%=rs("id")%>','<%=rs("item")%>','<%=rs("tid")%>')"   class="inputbutton"  style='color:FF6600;font-size=9pt' onMouseOver="this.style.color='#FF0000'" onMouseOut="this.style.color='#FF6600'">
					<%else%>
					<input  type="button" value="預約未到" <%if rs("yn")="N" then response.write "disabled" end if%>  onclick="absence_record_item('<%=rs("id")%>','<%=rs("item")%>','<%=rs("tid")%>')"   class="inputbutton"  style='color:#3300CC;font-size=9pt' onMouseOver="this.style.color='#3399FF'" onMouseOut="this.style.color='#3300CC'">
					<input  type="button" value="報到登入" <%if rs("yn")="N" then response.write "disabled" end if%> onclick="signin_record_item('<%=rs("id")%>','<%=rs("item")%>','<%=rs("tid")%>')" class="inputbutton"  style='color:009900;font-size=9pt' onMouseOver="this.style.color='#00cc00'" onMouseOut="this.style.color='#009900'">
					<%end if%>
				<%end if%>
				</TD>
			</TR>

...
if validate="add" then
	...
	if checkflag ="true" then
		if category="S" then
			...
			sql = "select   isnull(sum(summin),0) as summin  from  boo_book_software where  bdate='"&bdate&"' and sid='"&sid&"'  and  yn='Y' and category='S' "
			'response.write sql
			'response.end
...
function check_input()
{
	...
	if (errmsg=="")
	{
		...
		time1 =parseInt(form1.times.value);//可項約時間
...
function ChkStudent()
{
	vWinCal2 = window.open("lib/checkstudentfors.asp?sid="+form1.sid.value+"&category="+form1.category.value,"iframe_query", "width=200,height=50,status=no,resizable=no,scrollbars=no");
	//vWinCal2.opener = form1;
}
function itemchange()
{
	form1.times.value=form1.item.options[form1.item.selectedIndex].dname;
}
</script>
</HEAD>
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="form1" name="form1" method="post" action="booksoftware.asp" onsubmit="return check_input();">
			...
			<TR><TD>
				<TABLE cellSpacing=0 cellPadding=0  border=0 >
					<TR class="inputlabel">
						<TD>預約日期：</TD>
						<TD>預約時間起迄：(預約時數不能大於處方簽該項目之可預約時數)</TD>
					</TR>
					<TR>
						<TD valign="top">
...
'							Str=Str&"<TD>&nbsp;<input type=radio name='itemc' value="""&rsLoad("id")&""" checked></TD><TD>"&  rsLoad("floor")&"&nbsp; - &nbsp;"&  rsLoad("software")&"</TD>"
'						else
'							Str=Str&"<TD>&nbsp;<input type=radio name='itemc' value="""&rsLoad("id")&""" ></TD><TD>"&  rsLoad("floor")&"&nbsp;-&nbsp;"&  rsLoad("software") &"</TD>"
'						end if 
'						rsLoad.MoveNext 
'					wend
'					rsLoad.close
...
'					set rsLoad=nothing
					...
					<TR>
						...
						<TD><input type="text" value="0"  size="5" name="times" class="inputtext" style="BORDER-RIGHT:#f0f0f0 2px solid;BORDER-TOP:#f0f0f0 2px solid;BORDER-LEFT:#f0f0f0 2px solid;BORDER-BOTTOM:#f0f0f0 2px solid" readonly ></TD>
					</TR>

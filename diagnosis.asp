...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;學員診斷處方籤紀錄維護</TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<form id="news_form" name="news_form" method="post" action="diagnosis.asp" >
	...
	<%
		...
		sql = sql & " left join boo_diagnosis b on a.id=b.tid  where a.item='診斷'  and  a.signin is not null  "
		if teachername <>"" then
		...
		sql = sql & " order by bdate desc,btime "
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD width="32%">預約人</TD><TD width="13%">日期</TD><TD width="7%">時段</TD><TD width="17%">節數</TD><TD width="8%">大專英檢</TD><TD width="15%">教師</TD><TD width="8%" align="center">回診個案</TD>
			</TR>
			<TR class="inputlabel">
				<TD></TD><TD colspan="2">診斷內容</TD><TD colspan="4">建議</TD><TD></TD>
			</TR>
			<TR>
				<TD height="1" bgcolor="#000000" colspan="11"></TD>
			</TR>
			...
			<TR bgcolor="<%=vcolor%>">
				<TD>
				<a href="diagnosisedit.asp?id=<%=rs("bookid")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a>
				</TD>
				...
				<TD  align="center"><%=rs("backcase")%></TD>
			</TR>
			<TR bgcolor="<%=vcolor%>">
				<TD></TD><TD colspan="2"><%=rs("content")%>&nbsp;</TD>
				<TD colspan="4">
				<%
				comStr = ""
				if rs("op")="Y" then 
					comStr = comStr &  "口語：" &  rs("optime")  & "次"
				end if
				if rs("pp")="Y" then 
					if comStr<>"" then  comStr = comStr & "　" end if
					comStr = comStr &  "簡報：" &  rs("pptime")  & "次"
				end if
				if rs("crkp")="Y" then 
					if comStr<>"" then  comStr = comStr & "　" end if
					comStr = comStr &  "詩歌：" &  rs("crkptime")  & "次"
				end if
				
				if rs("write")="Y" then 
					if comStr<>"" then  comStr = comStr & "　" end if
					comStr = comStr &  "寫作：" &  rs("writetime")  & "次"
				end if
				if rs("reading")="Y" then 
					if comStr<>"" then  comStr = comStr & "　" end if
					comStr = comStr &  "閱讀：" &  rs("readtime")  & "次"
				end if
				if rs("soft")="Y" then 
					if comStr<>"" then  comStr = comStr & "　" end if
					comStr = comStr &  "自學軟體：" &  rs("softtime")  & "小時"
				end if
				if rs("extra")="Y" then 
					if comStr<>"" then  comStr = comStr & "　" end if
					comStr = comStr &  "補充教材：" &  rs("extratime")  & "小時"
				end if

				response.write comStr
				%>
				</TD>
				<TD align="right"><a href="recordprofile.asp?sid=<%=rs("sid")%>&sender=<%=sender%>"><img border="0"  src="images/record.gif"></a></TD>
			</TR>
			<TR>

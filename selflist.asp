...
sender=server.urlencode(replace(request.servervariables("PATH_INFO")&"?page="&page& "&item=" & item& "&category=" & category& "&rank=" & rank,"%","*"))
...
<!-- ---------------------------------------------------------------------------------------- -->
	...
	<TR >
		<TD WIDTH="5" height="20"></TD><TD class="T3" valign="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;自學療程項目維護 </TD>
	</TR>
	<TR >
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR><TD></TD>
				...
				<TD><input  type="submit" value="查詢" class="inputbutton"><input  type="button"  onclick="window.location='selfadd.asp'" value="新增" class="inputbutton"></TD>
			</TR>
			</form>
			</TABLE>
		</TD>
	...
	<%
		...
		sql = "select * from boo_self_item a where 1=1  "
		
		if topic<>"" then
			sql = sql & " and a.item='"&item&"' "
		end if
	...
	<TR>
		<TD></TD><TD valign="top">
			...
			<TR class="inputlabel">
				<TD></TD><TD>項目</TD><TD>一天限定時數</TD><TD>開放否</TD><TD></TD><TD></TD>
			</TR>
			<TR>
				<TD height="1" bgcolor="#000000" colspan="11"></TD>
			</TR>
			...
			<TR bgcolor="<%=vcolor%>">
				...
				<TD><a href="selfedit.asp?id=<%=rs("id")%>&sender=<%=sender%>"><img border="0" src="/include/lib/images/wri.gif"></a></TD><TD><%=rs("item")%></TD>
				<TD><%=rs("hours")%></TD>
